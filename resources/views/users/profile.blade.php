      ...
      <div class="col-lg-12">
        <div class="w-100 text-center py-4 img-profile-user">
          ...
          @endif
            ...
            <img src="{{Helper::getFile(config('path.avatar').$user->avatar)}}" width="150" height="150" alt="{{$user->name}}" class="rounded-circle img-user mb-2 avatarUser">
          </div><!-- avatar-wrap -->
          ...
          @endif
            ...
            @if (Auth::check() && Auth::user()->id != $user->id && $user->updates()->count() <> 0)
              <a href="javascript:void(0);" data-toggle="modal" title="{{trans('general.tip')}}" data-target="#tipForm" class="btn btn-google btn-profile mr-1" data-cover="{{Helper::getFile(config('path.cover').$user->cover)}}" data-avatar="{{Helper::getFile(config('path.avatar').$user->avatar)}}" data-name="{{$user->name}}" data-userid="{{$user->id}}">
                ...
                <div class="modal-body">
          				<div class="container-fluid">
          					<div class="row">
          						...
          						<div class="col-md-4 col-6 mb-3">
          							<a href="https://twitter.com/intent/tweet?url={{url()->current()}}&text={{ e( $user->name ) }}" data-url="{{url()->current()}}" class="social-share text-muted d-block text-center h6" target="_blank" title="Twitter">
          						...
          						<div class="col-md-4 col-6 mb-3">
          							<a href="mailto:?subject={{ e( $user->name ) }}&amp;body={{url()->current()}}" class="social-share text-muted d-block text-center h6" title="{{trans('auth.email')}}">
            ...
            <div class="card bg-white shadow border-0">
              ...
              <div class="card-body px-lg-5 py-lg-5 position-relative">
                ...
                <div class="text-muted text-center mb-3 position-relative modal-offset">
                  <img src="{{Helper::getFile(config('path.avatar').$user->avatar)}}" width="100" alt="{{$user->name}}" class="avatar-modal rounded-circle mb-1">
                  <h6 class="font-weight-light">
                    {!! trans('general.get_access_month', ['price' => '<span class="font-weight-bold">'.Helper::amountWithoutFormat($user->price).'</span>']) !!} {{trans('general.unlocked_content')}} {{$user->name}}
                  </h6>
                </div>
                ...
                <form method="post" action="{{url('buy/subscription')}}" id="formSubscription">
                  ...
                  @foreach ($allPayment as $payment)
                    ...
                    @if ($payment->name == 'Stripe' && ! auth()->user()->stripe_id != '')
            ...
            <div class="card bg-white shadow border-0">
              ...
              <div class="card-body px-lg-5 py-lg-5 position-relative">
                ...
                <div class="text-muted text-center mb-3 position-relative modal-offset">
                  <img src="{{Helper::getFile(config('path.avatar').$user->avatar)}}" width="100" alt="{{$user->name}}" class="avatar-modal rounded-circle mb-1">
                  <h6 class="font-weight-light">
                    {{trans('general.subscribe_free_content') }} {{$user->name}}
                  </h6>
                </div>

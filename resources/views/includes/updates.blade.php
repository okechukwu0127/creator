	...
	<div class="media">
		<span class="rounded-circle mr-3">
			<a href="{{url($response->user()->username)}}">
				<img src="{{ Helper::getFile(config('path.avatar').$response->user()->avatar) }}" alt="{{$response->user()->name}}" class="rounded-circle avatarUser" width="60" height="60">
				</a>
		</span>
		...
		<div class="media-body">
			...
			@endif
				...
				@if(Auth::check() && Auth::user()->id != $response->user()->id && $response->locked == 'yes'
					&& auth()->user()->checkSubscription($response->user())
...
@else
	...
	<div class="btn-block p-sm text-center content-locked pt-lg pb-lg">
		...
		@if ($response->user()->price != 0.00 || $response->user()->free_subscription == 'yes')
